...
class Notice : Fragment() {
    ...
    private class MyHandler(fragment: Notice): Handler() {
        private val mFragment: WeakReference<Notice> = WeakReference(fragment)
    ...
    private fun networkTask() {
        val mHandler = MyHandler(this@Notice)
        object: Thread() {
            override fun run() {
                mHandler.post { mSwipeRefreshLayout.isRefreshing = true }
                try {
                    val doc = Jsoup.connect("http://www.sawl.hs.kr/sys/bbs/board.php?bo_table=030601").get()
                    val titleData = doc.select("#fboardlist div table tbody tr td.td_subject a")
                    val authorData = doc.select("#fboardlist div table tbody tr td.td_name.sv_use span.sv_member")
                    val dateData = doc.select("#fboardlist div table tbody tr td.td_date")
